...
def register_endpoints(app):
    ...
    def create_dream_endpoint(args, userEmail):
        try:
            ...
            # Here, make sure the 'dream' dictionary contains the UUID generated by 'create_memory'
            uuid_from_dream = dream.get("id", None)

            if uuid_from_dream is None:
                log(f"UUID is missing from the created dream. Aborting...", type="error")
                return jsonify({"error": "UUID is missing from the created dream"}), 500

            log(f"Successfully created dream with UUID {uuid_from_dream} and data {dream}", type="info")

            # You can add the UUID to the response if you like
            response_data = {"uuid": uuid_from_dream, **dream}
