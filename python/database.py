...
def get_dream_analysis(dream_id, max_retries=5):
    try:
        ...
        for _ in range(max_retries):
            ...
            time.sleep(5)
        log(
            f"Failed to get dream analysis after {max_retries} attempts.", type='error', color='red')
        return None
...
def get_dream_image(dream_id, max_retries=5):
    try:
        ...
        for _ in range(max_retries):
            ...
            time.sleep(5)
        log(
            f"Failed to get dream image after {max_retries} attempts.", type='error', color='red')
        return None
    except Exception as e:
...
def update_dream_analysis_and_image(dream_id, analysis=None, image=None):
    log(
        f'Updating dream analysis and image for dream id {dream_id}.', type='info')
    dream = get_dream(dream_id)
    if dream is None:
        log(f"Dream with id {dream_id} not found.", type='error', color='red')
        return None
