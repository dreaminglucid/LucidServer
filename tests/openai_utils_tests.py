...
import pytest
from openai_utils import get_image_summary, generate_dream_analysis, generate_dream_image, regular_chat, search_dreams, call_function_by_name, search_chat_with_dreams
...
def test_get_image_summary_success():
    with patch('openai_utils.text_completion', side_effect=mock_text_completion_summary):
...
def test_get_image_summary_gpt4_error():
    with patch('openai_utils.text_completion', side_effect=mock_text_completion_with_error):
        dream_entry = "A mysterious dream about a forest"
        result = get_image_summary(dream_entry)
...
def test_get_image_summary_exception():
    with patch('openai_utils.text_completion', side_effect=Exception("An unexpected exception")):
        dream_entry = "A mysterious dream about a forest"
        result = get_image_summary(dream_entry)
        assert result == "Error: Unable to generate a summary.", f"Expected error message, but got {result}"
...
def test_generate_dream_analysis_success():
    with patch('openai_utils.text_completion', side_effect=mock_text_completion_analysis):
...
def test_generate_dream_image_success():
    with patch('openai_utils.get_image_summary', return_value="Generated Summary"):
...
def test_generate_dream_image_exception():
    with patch('openai_utils.get_image_summary', side_effect=Exception("An unexpected exception")):
        dreams = [{"id": "1", "metadata": {"entry": "A mysterious dream about a forest"}}]
        dream_id = "1"
        result = generate_dream_image(dreams, dream_id)
        assert result is None, f"Expected None, but got {result}"
...
def test_regular_chat_success():
    with patch('openai_utils.chat_completion', side_effect=mock_chat_completion):
...
def test_regular_chat_gpt4_error():
    with patch('openai_utils.chat_completion', side_effect=mock_chat_completion_with_error):
        message = "Tell me more about lucid dreaming techniques."
        user_email = "user@example.com"
        result = regular_chat(message, user_email)
        assert result == "Error: Unable to generate a response.", f"Expected error message, but got {result}"
...
def test_regular_chat_exception():
    with patch('openai_utils.chat_completion', side_effect=Exception("An unexpected exception")):
        message = "Tell me more about lucid dreaming techniques."
        user_email = "user@example.com"
        result = regular_chat(message, user_email)
        assert result == "Error: Unable to generate a response.", f"Expected error message, but got {result}"
...
def test_search_dreams():
    with patch('openai_utils.search_memory', side_effect=mock_search_memory):
        keyword = "Dream"
        user_email = "user@example.com"
...
def test_call_function_by_name():
    with patch('openai_utils.function_completion', side_effect=mock_function_completion):
...
def test_search_chat_with_dreams_with_results():
    with patch('openai_utils.search_dreams', side_effect=mock_search_memory), \
         patch('openai_utils.count_tokens', side_effect=mock_count_tokens), \
         patch('openai_utils.call_function_by_name', side_effect=mock_function_completion):
        function_name = "discuss_emotions"
        prompt = "Discuss emotions in dreams"
        user_email = "user@example.com"
...
def test_search_chat_with_dreams_without_results():
    with patch('openai_utils.search_memory', side_effect=lambda *args, **kwargs: []), \
         patch('openai_utils.count_tokens', side_effect=mock_count_tokens), \
         patch('openai_utils.call_function_by_name', side_effect=mock_function_completion):
        function_name = "discuss_emotions"
        prompt = "Discuss emotions in dreams"
